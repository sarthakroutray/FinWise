...
def extract_final_chart(response: str, env: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Extract chart data from FINAL_CHART() statement.
    ...
    """
    if 'FINAL_CHART(' not in response:
        return None
        
    # Get the chart data from the environment (set by plot() helper)
    if '_chart' in env:
        return env['_chart']
...
def is_final(response: str) -> bool:
    """
    Check if response contains FINAL(), FINAL_VAR(), or FINAL_CHART().
    ...
    """
    return any(keyword in response for keyword in ['FINAL(', 'FINAL_VAR(', 'FINAL_CHART('])


def parse_response(response: str, env: Dict[str, Any]) -> Optional[Any]:
    """
    ...
    Returns:
        Final answer, chart data dict, or None
    """
    # Try FINAL_CHART() first (high priority for visualization)
    chart = extract_final_chart(response, env)
    if chart is not None:
        return {"type": "chart", "payload": chart}
