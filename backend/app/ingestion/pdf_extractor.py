...
# Path to the downloaded poppler binaries for Windows OCR
POPPLER_PATH = str(Path(__file__).parent.parent.parent / "poppler" / "poppler-24.08.0" / "Library" / "bin")
...
class BankStatementPDFExtractor:
    ...
    def _extract_text_ocr(self, path: str) -> List[str]:
        ...
        try:
            ...
            if platform.system() == "Windows" and Path(POPPLER_PATH).exists():
            ...
            # Fallback if tesseract or poppler isn't installed
            print(f"OCR Extraction failed: {e}. Falling back to native text extraction.")
